...
import { exchangeWorkspaceCode, getConnectedEmail } from "@/lib/google-workspace";
...
export async function GET(req: NextRequest) {
  ...
  const fail = (reason: string) =>
    NextResponse.redirect(new URL(`${DEST}?gws_error=${encodeURIComponent(reason)}`, req.url));

  if (error) return fail(error);
  if (!code || !state) return fail("missing_params");

  const cookieStore = await cookies();
  const storedState = cookieStore.get("workspace_google_oauth_state")?.value;
  cookieStore.delete("workspace_google_oauth_state");
  ...
  try {
    ...
    const { error: dbError } = await supabaseAdmin.from("google_workspace_connection").insert({
    ...
    return NextResponse.redirect(new URL(`${DEST}?gws_connected=1`, req.url));
  } catch (err) {
